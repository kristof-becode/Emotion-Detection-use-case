...
MASK_MODEL_PATH=os.getcwd()
FACE_MODEL_PATH=os.getcwd()
...
maskNet = load_model('emotion_model.h5')

# initialize the video stream and allow the camera sensor to warm up
print("[INFO] starting video stream...")
#vs = VideoStream(0).start()
fourcc = cv2.VideoWriter_fourcc('V','P','8','0')
vs = FileVideoStream('videos/robin.mp4').start()
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		if label == "anger":
			...
			cv2.rectangle(original_frame, (startX, startY), (endX, endY), (0, 50, 200), 2)
		print(label)
		#emotion_label = emotion_label.append(label)
	...
	if key == ord("q"):
		#print(emotion_label)
		#data = list(zip(timestamp, emotion_label))
